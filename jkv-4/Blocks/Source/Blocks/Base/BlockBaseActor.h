...
public:	
	...
	/* Add a Ghost Mesh Variable allow the GhostMesh to be edited in the editor or in code */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="Ghost")
        UStaticMeshComponent* GhostMesh;
    
	/* ABlockBaseActor* because we’re referencing another block */
	UPROPERTY()
    ABlockBaseActor* PotentialMergeTarget = nullptr;

    /* Variable for rotation of the block*/
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="Ghost")
