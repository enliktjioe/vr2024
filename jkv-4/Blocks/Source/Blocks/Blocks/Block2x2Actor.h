...
class BLOCKS_API ABlock2x2Actor : public ABlockBaseActor
{
	...
	ABlock2x2Actor() {
		...
		// 2) GHOST MESH: same shape, but translucent
    	static ConstructorHelpers::FObjectFinder<UStaticMesh> GhostMeshFinder(TEXT("StaticMesh'/Game/Blocks/Meshes/Block2x2.Block2x2'"));
    	if (GhostMeshFinder.Succeeded() && GhostMesh != nullptr) {
        	GhostMesh->SetStaticMesh(GhostMeshFinder.Object);
    	}

		// 3) Load the ghost material (M_GhostBlock)
    	static ConstructorHelpers::FObjectFinder<UMaterialInterface> GhostMatFinder(TEXT("Material'/Game/Blocks/Meshes/M_GhostBlock.M_GhostBlock'"));
    	if (GhostMatFinder.Succeeded() && GhostMesh != nullptr)
    	{
        	GhostMesh->SetMaterial(0, GhostMatFinder.Object);
    	}
